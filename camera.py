...
class Camera:
    ...
    def __getStreams(self):
        if not self.videoName:  # stream
            ...
            try:
                ...
                raise FileNotFoundError("No available camera detected.")
            ...
            # Handle images from folder
            pass

        elif self.videoName[-4:] in [".mp4", ".avi", ".mov"]:
    ...
    # throws EOFError when end of video
    def getFrame(self):
        if self.vType == 'builtin':
            ret, frame = self.vid.read()
            if not ret:
                raise EOFError("No more frames.")
            return frame, None  # Return frame and None for depth
        elif self.vType == 'bag':
            # Handle bag file
            pass
        elif self.vType == 'rs':
